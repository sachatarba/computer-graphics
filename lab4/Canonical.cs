...
namespace lab4
{
    internal class Canonical
    {
        ...
        public static List<Point> GetPointsForEllipse(Point center, int heigth, int width)
        {
            ...
            {
                y = Convert.ToInt32(center.Y + Math.Sqrt(Math.Abs(sqrWidth * sqrHeight - (x - center.X) * (x - center.X) * sqrHeight)) / width);
                points.AddRange(Painter.ReflectPointsOfEllipse(new Point(x, y), center));
            }

            for (int y = center.Y, x = 0; y < heightEnd + 1; ++y)
            {
                x = Convert.ToInt32(center.X + Math.Sqrt(Math.Abs(sqrWidth * sqrHeight - (y - center.Y) * (y - center.Y) * sqrWidth)) / heigth);
                points.AddRange(Painter.ReflectPointsOfEllipse(new Point(x, y), center));
            }
